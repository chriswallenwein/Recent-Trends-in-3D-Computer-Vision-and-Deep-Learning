...
The underlying optics concepts used in physics-based papers are fundamental to understand "Through the looking glass". The section “Overview of optics fundamentals” will introduce these topics.
...
## Normal prediction network [work in progress]
To estimate the normals for , we will use an encoder-decoder CNN. The cost volume is still too big to be fed into the network. Therefore, we first use learnable pooling to perform feature extraction on the cost volume. We concatenate the condensed cost volume together with
...
and feed everything to the encoder - decoder CNN. We use (insert loss function here) as the loss function. It is simply the L2 distance between the estimated normals and the ground truth normals.
Point cloud reconstruction
The normal prediction network gives us important information about the transparent object from different viewing angles. But somehow we have to match the features in the different views with the points in the point cloud. After we mapped the features, a modified PointNet++ will predict the final point locations and final normal vectors for each point in the point cloud. Finally the 3D point cloud will be transformed into a mesh by applying poisson surface reconstruction.
Feature mapping
The goal of feature mapping is to assign features to each point in the initial point cloud. More specifically we care about the normal at that point, the rendering error and if total internal reflection happened at that point.
Map point in 3D space onto each image to figure out the value at this point. Sometimes a point might not be visible from a particular angle, if that is the case, we will not take it into account during feature mapping.
For each point we usually have 10 different views and predictions, we have to decide from which view we should create our feature vectors. The authors try three different approaches:
...
## Modified PointNet++ [first half of section done]
...
[the part of the section below is not done yet.]
To get the best results, the authors tried three different loss functions
Nearest view based loss
...
Poisson surface reconstruction was first introduced in 2006 and is still used in recent papers. It takes a point cloud and surface normal estimations for each point of the point cloud and reconstructs the 3D mesh of the object.
...
## Comparison to SOTA
Quick reminder: this is deep learning research. It comes at no big surprise, that there was a newer paper [10] with a different approach that works better. This newer paper is the successor of [4] Here we can see the qualitative results of a new paper . On the left side we can see the results from this paper (1st column) compared with their ground truth (2nd column). On the right site we can see the results of the newer paper (4th column) compared with their ground truth (3rd column).
...
For the sake of simplicity, I left out some important details. If you have any questions, take a look at the paper [1], the code of the paper [2] or send me an email.
Thanks to Pengyuan and Yu-Ying Yeh for their generous support.
